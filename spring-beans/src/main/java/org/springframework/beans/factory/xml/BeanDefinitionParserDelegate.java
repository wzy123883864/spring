...
public class BeanDefinitionParserDelegate {
	 ...
	 *2.解析 id，name属性放入List aliases中；aliases持有所有别名
	 *3.检查beanName，和aliases是否跟之前有重复；因为当前的对象BeanDefinitionParserDelegate中属性 Set usedNames 会持有所有解析出来的beanName 和 aliases；
	...
	public AbstractBeanDefinition parseBeanDefinitionElement(
		 ...
		 * 2.ParseState类里面有一个private final Stack state;栈；存放的对象类型是 Entry
		...
		try {
			...
			/**
			 *1.解析 里面的所有属性值；例如:scope,abstract,lazy-init,autowire,primary,depends-on等
			 *2.所有的属性都设置到了 AbstractBeanDefinition bd中
			 */
			parseBeanDefinitionAttributes(ele, beanName, containingBean, bd);
			//设置description值；例如test
			bd.setDescription(DomUtils.getChildElementValueByTagName(ele, DESCRIPTION_ELEMENT));

			/**
			 *1.解析元数据 Meta； Meta在Element中是
			...
			/**
			 *1.解析构造函数中的参数 constructor-arg；放入bd中的 ConstructorArgumentValues constructorArgumentValues属性中Map indexedArgumentValues map中;
			 *2.解析的是 这种的
			 */
			parseConstructorArgElements(ele, bd);
			/**
			 *1.set方法注入的处理(比较常用的注入)；格式是
			...
			/**
			 *1.解析qualifier元素；存放在 Map qualifiers
			 */
